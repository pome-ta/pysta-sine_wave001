...
kAudioUnitType_Output = int.from_bytes(
  b'auou', byteorder='big')
kAudioUnitSubType_RemoteIO = int.from_bytes(
  b'rioc', byteorder='big')
kAudioUnitManufacturer_Apple = int.from_bytes(
  b'appl', byteorder='big')
# --- defaultOutputDescription */
...
def render_callback_prototype(
    ...
    inTimeStamp: ctypes.POINTER(AudioTimeStamp),
    inBusNumber: ctypes.c_uint32,
    inNumberFrames: ctypes.c_uint32,
    ioData: ctypes.POINTER(AudioBufferList)) -> ctypes.c_uint32:
  pass

AURenderCallbackargs = list(
  render_callback_prototype.__annotations__.values()
  )
AURenderCallback = ctypes.CFUNCTYPE(
  AURenderCallbackargs[-1],
...
defaultOutput = AudioComponentFindNext(
  None, ctypes.byref(defaultOutputDescription)
  )

toneUnit = ctypes.c_void_p(0)
err = AudioComponentInstanceNew(
  defaultOutput, ctypes.byref(toneUnit)
  )
